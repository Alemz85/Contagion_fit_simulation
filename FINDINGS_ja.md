...
| **Watts-Dodds ①**：大カスケードはインフルエンサーでなく臨界量 | **ハブ起点（インフルエンサー）**と**ランダム起点（一般大衆）**でリーチが逆転する領域はあるか | **シード戦略実験** hub vs random、(p,φ)平面の**反転境界** | §3.4 / F4・F5 | 単一シードではハブ優位（限界⑤の留保つき） |
...
- **パラメータ回収テスト**（[test_fit.py](tests/test_fit.py)）：IC自身で生成した合成データをグリッドサーチに食わせ、真の `p` を回収できるか確認 → 手法の妥当性を担保。pytest 24件すべてパス。
...
- **両基盤とも「ランダム優位」領域は出現せず**。単一シードの平均リーチではハブがランダムを下回らないため。Watts-Doddsの分散シード優位の厳密検証には**複数シード＋コスト正規化**が必要 → 次のステップ。
...
5. **単一シードの制約**: シード戦略の結論は単一シードの平均リーチに基づく。Watts-Dodds検証には複数シード・コスト正規化が必要。
...
| 種類 | パス |
|---|---|
| BA基盤の図・結果 | `results/`（F1〜F6, summary.json） |
| WS基盤の図・結果 | `results_ws/`（F1〜F5, summary.json） |
...
- **複数シード＋コスト正規化**でWatts-Doddsの分散シード優位を厳密に検証
