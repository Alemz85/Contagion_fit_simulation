...
| **Watts-Dodds ①**: large cascades come from critical mass, not influentials | Is there a region where **hub seeding (influencers)** and **random seeding (the crowd)** swap in reach? | **Seeding experiment** hub vs random, the **flip boundary** over the (p,φ) plane, and the **budgeted seeding experiment** (§4.4 / F8) | §3.4·§4.4 / F4·F5·F8 | Hubs dominate under single seeds, but a fair budget+cost test (§4.4) shows **distributed seeding wins under simple contagion** |
...
- **Parameter-recovery test** ([test_fit.py](tests/test_fit.py)): feed synthetic data generated by IC itself into the grid search and check the true `p` is recovered → guarantees the soundness of the method. All 31 pytest cases pass (24 base + 7 for the budgeted seeding extension, §4.4).
...
- **Neither substrate shows a "random-advantage" region under single seeds.** Under single-seed mean reach a hub never underperforms a random node — but this is an unfair test (one celebrity vs one ordinary account). A rigorous test of Watts-Dodds' distributed-seeding advantage needs **multiple seeds + cost normalization**, which is carried out in **§4.4** below (F8).
...
### 4.4 Budgeted seeding — the fair Watts-Dodds test (F8)

The single-seed experiment (§4.3) is structurally unfair: comparing one hub to
one random node, a hub can never lose on mean reach. The real Watts-Dodds
question is whether, **for a fixed budget**, a distributed crowd of cheap
ordinary accounts out-reaches one expensive celebrity. We make the contest fair
([experiments.py](src/contagion_fit/experiments.py) `seed_budget_experiment`):

- **Multiple seeds under a budget**, not one. A fixed budget is spent on as many
  seeds as it can afford.
- **Seed price rises with degree**: `cost(v) = 1 + α · degree(v) / mean_degree`
  ([network.py](src/contagion_fit/network.py) `degree_cost`). One mega-hub costs
  roughly what a dozen ordinary accounts cost, so "10 micro-influencers vs 1
  celebrity" is a fair fight. (`α = 0` recovers a plain k-seed budget.)
- **Five strategies compete** on reach **and reach-per-cost**: `random`, `hub`,
  `pagerank`, `betweenness`, and **greedy influence maximisation**
  (Kempe-Kleinberg-Tardos: iteratively add the node with the largest marginal
  expected reach per unit cost, evaluated by the simulator itself), all driven by
  the same seeded `Generator` so the result is reproducible.

**Result (BA substrate, budget 15, α = 1; F8).** The answer depends on the
mechanism, mirroring the substrate-flip story:

| Mechanism | Single expensive hub (k=1) | Distributed cheap crowd | Winner |
|---|---|---|---|
| **IC (simple)** | reach ≈ 9–10 | random (k≈7) ≈ 13–14; **greedy (k≈14) ≈ 17** | **distributed wins** on reach *and* reach-per-cost |
| **Threshold (complex)** | hub/targeted ignite globally (≈ n) | random (k≈7) sometimes fizzles (≈ 0.6 n) | targeting wins reach; the global-ignition strategies are **near-tied** on reach-per-cost, random lags |

- **Under simple contagion there IS a distributed-advantage region**: the hub
  burns almost the whole budget on a single account, while the same budget buys
  ~8 cheap accounts that reach further. Crucially, the **greedy optimiser, given
  a candidate pool that includes cheap nodes, independently discovers the
  cheap-distributed set** — confirming that the optimum is the crowd, not the
  celebrity. This is exactly the Watts-Dodds claim, which the single-seed setup
  in §4.3 could not reveal.
- **Under complex contagion** a seed set must hit the reinforcement structure to
  ignite, so hub/targeted seeding reaches more; once ignited the targeted
  strategies (hub / pagerank / betweenness / greedy) are essentially tied on
  reach-per-cost, while random lags because it sometimes fails to ignite at all.

| IC (simple): distributed crowd wins | Threshold (complex): targeting wins reach |
|---|---|
| ![F8 budget IC](results/F8_budget_cost_ic.png) | ![F8 budget threshold](results/F8_budget_cost_threshold.png) |

*Caveat (honest):* greedy evaluates a candidate pool of the top-degree nodes
**plus a random sample** of others (not all n nodes) to stay tractable on large
substrates, so it is KKT greedy over a sampled candidate set.
...
5. **Single-seed constraint** — *addressed in §4.4 (F8)*: the §4.3 seeding conclusion was based on single-seed mean reach, an unfair test. The budgeted seeding experiment (multiple seeds + degree-based cost normalization + greedy influence maximisation) now tests Watts-Dodds fairly and finds a genuine distributed-advantage region under simple contagion. Remaining gap: results shown on the BA substrate at one budget/α; a full budget×α sweep across both substrates is the natural extension.
...
| Type | Path |
|---|---|
| BA substrate figures/results | `results/` (F1–F6, **F8 budget/cost**, summary.json) |
...
| Tests | `tests/` (31 cases, all passing — incl. 7 for the budgeted seeding extension) |
...
- ~~Multiple seeds + cost normalization to rigorously test the Watts-Dodds distributed-seeding advantage.~~ **Done — see §4.4 (F8).** Natural follow-on: sweep budget and cost-slope α across both BA and WS substrates and map where the distributed-advantage region opens and closes.
